...
| `comment-code-span.json` | `GH api repos/disciplin-run-org/jjstack/issues/comments/5624626836 -H "Accept: application/vnd.github.full+json" --jq '{id, created_at, user: {login: .user.login}, author_association, body, body_html}'`, the round-1 response on #49. The handle appears in it only in backticks, and GitHub's `body_html` has no mention link for it |
| `markdown-*.html` | `gh api -X POST markdown` with `{"text": T, "mode": "gfm", "context": "disciplin-run-org/jjstack"}`: GitHub's own rendering. T for each file: `mention` is `@ai-assistant-2026 please review`; `quote` is `> @ai-assistant-2026 quoted`; `crlf-fence` is `Fixed the parser:\r\n\r\n` + a CRLF code block holding `code` + `\r\n\r\n@ai-assistant-2026 please review`; `other-user` is `@JesperJurcenoks please review`; `stray-backtick` is ``Don't merge ` yet.\n\n@ai-assistant-2026 please review `bin/x` `` (a lone backtick, a paragraph break, the mention, a code span); `fence` is `see:\n` then an LF code block holding `@ai-assistant-2026 review`; `quote-reply` is `> @ai-assistant-2026 please review this\n\nNo, we will not take this PR.`; `nested-quote` is `> > @ai-assistant-2026 nested\n> outer`; `after-quote` is `> quoted words\n\n@ai-assistant-2026 please review` |
