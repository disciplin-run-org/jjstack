...
MUTANTS = [
    ...
    ("mention-ungated", "if perm in WRITE_PERMISSIONS:", "if True:"),
    ("any-bump-is-a-request", "if asked and max(asked) > last_round:", "if True:"),
    ("cleared-request-accepted", "if REVIEWER in wanted:", "if True:"),
    ("first-page-only", "value, i = dec.raw_decode(text, i)", "value, i = dec.raw_decode(text, i)[0], n"),
    ("stale-mention-counts", "if at > floor and who", "if who"),
    ("failed-lookup-trusted", 'return "unknown: %s" % e', 'return "write"'),
    ("failed-lookup-marks-read", "        if undecided:\n", "        if False:\n"),
    ("mention-any-user", "if href and href.group(1).lower() == REVIEWER:", "if href:"),
    ("mention-class-ignored", r'''<a\s[^<>]*?\bclass="[^"<>]*\buser-mention\b[^"<>]*"[^<>]*>''', r"<a\s[^<>]*>"),
    ("raw-markdown-read", 'return obj.get("body_html")', 'return obj.get("body")'),
    ("no-body-html-silent", 'if "body_html" not in obj and obj.get("body"):', "if False:"),
    ("quotes-count-again", "        elif depth == 0:\n", "        else:\n"),
    ("depth-never-closes", "depth = max(0, depth + (-1 if m.group(1) else 1))", "depth = depth + 1"),
    ("pull-no-html-accept", "number), headers=(HTML_ACCEPT,))", "number))"),
    ("list-no-html-accept", "self._get_list(path, headers=(HTML_ACCEPT,))", "self._get_list(path)"),
    ("get-list-drops-headers", "self.gh.api(path, headers=headers, paginate=True)",
     "self.gh.api(path, paginate=True)"),
    ("ims-not-reset",
     "                self.ledger.last_modified = None  # re-list next poll; the thread stays unread\n", ""),
    ("warn-key-without-update", '"act:%s:%s" % (t["thread_id"], t["updated_at"])', '"act:%s" % t["thread_id"]'),
    ("raise-at-first-failure", 'if perm.startswith("unknown"):\n                undecided.append(login)',
     'if perm.startswith("unknown"):\n                raise GHError(perm)'),
    ("404-retried", 'if "HTTP 404" in str(e):', "if False:"),
