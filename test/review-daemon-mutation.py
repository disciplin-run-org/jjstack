...
MUTANTS = [
    ...
    ("window-always-held", "WINDOW_SCRIPT = '\"$0\" \"$@\" || exec bash'",
     "WINDOW_SCRIPT = '\"$0\" \"$@\"; exec bash'"),
    ("window-never-held", "WINDOW_SCRIPT = '\"$0\" \"$@\" || exec bash'", "WINDOW_SCRIPT = '\"$0\" \"$@\"'"),
    ("reason-filters-threads", 'if subject.get("type") != "PullRequest":',
     'if subject.get("type") != "PullRequest" or t.get("reason") != "review_requested":'),
    ("any-bump-is-a-request", 'asked = [e for e in asked if parse_iso(e["created_at"]) > last_round]', "pass"),
    ("cleared-request-accepted", "if REVIEWER not in wanted:", "if False:"),
    ("requester-ungated", "if perm in WRITE_PERMISSIONS:", "if True:"),
    ("any-old-request-stands", "asked = asked[:1]  # the request that stands is the newest one", "pass"),
    ...
    ("clean-exit-from-worker-row", 'roster.get(worker + "-manager")', "roster.get(worker)"),
    ("unclean-end-silent", '        if rec.get("exit_sent_at") and not clean:\n', "        if False:\n"),
