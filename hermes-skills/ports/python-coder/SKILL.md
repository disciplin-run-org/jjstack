...
def sample_behavior_spec():
    ...
    from my_package.models import BehaviorSpec
...
### Pattern 7: Subprocess Mock

```python
@pytest.fixture
def mock_subprocess(mocker):
    """Mock subprocess.run for external command invocation."""
    mock_run = mocker.patch("my_package.runner.subprocess.run")
...
### Pattern 8: Parametrize for Boundary Coverage

Use `@pytest.mark.parametrize` to implement boundary value analysis without
writing N separate test functions:
...
### Pattern 10: Hypothesis Property-Based Testing
...
### Pattern 11: mutmut Mutation Testing
...
Target 70%+ mutation score to start, iterate toward 85%.

---

## Self-Review Checklist — Run Before Delivering Code

Before presenting any Python code, verify all items:

1. Every significant `if`/`for`/`while`/`try`/`with` block has its `#end` comment
2. Imports grouped with `# Standard Libraries` / `# 3rd party` / `# Local` comments
3. Functions declared top-down (helpers above callers, `main()` last)
4. Copyright header present (if project uses one)
...
13. pytest patterns from this skill applied (fixtures, parametrize, conftest, Hypothesis)
