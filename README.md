...
| `/review` accountability | Dropped findings vanish | **Triage ledger**: every finding keeps a disposition + reason code, deduped with a corroboration count and a blast-radius label. Completeness is checked by matching each adjudicated finding to a ledger row of the same location AND claim, so nothing is dropped silently and nothing is accounted for by an unrelated row |
...
- [NVIDIA/SkillSpector](https://github.com/NVIDIA/SkillSpector) — the enrich-only verification architecture, baseline suppression, and three-valued verdict behind `/review`
