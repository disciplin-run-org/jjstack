...
## Phase 4.5: Cross-file blast radius + stale-API check

Two passes derived from the Macroscope research
(`references/vendor-lessons-macroscope.md`, which also records what was rejected
and why). Both are backed by deterministic scripts — the mechanical half is
mechanical, so no model does it by hand. Run them after the Phase 4 merge and
before Phase 5 scoring.

### 4.5a Blast radius — the caller the diff never showed you

Every pass so far reads the diff. That is right for scope and blind to exactly
one class: **the caller that was not updated.** Change a signature, a return
contract, a nullability or an error path, and the defect lives in files the diff
never touched.

```bash
~/.claude/skills/jjstack/bin/jjstack-review-blast-radius --base {BASE_REF}
```

The script lists definitions this diff changed and every file referencing them,
split into `in-diff` and `NOT-in-diff`. **Open the `NOT-in-diff` referrers** and
check each against the changed definition: does it still pass the right
arguments, handle the new return/error shape, respect the new invariant? Each
mismatch is a Phase 4 finding (lens: `blast-radius`) and goes into Phase 5
scoring like any other.

This is a **recall instrument** — it widens the net and suppresses nothing. It is
a grep census, not a type-resolved reference graph, so same-named symbols in
unrelated scopes will appear; verify before believing. Exit 3 (not a git repo) or
an empty census: note it and continue.
