...
- **Rebuilds are a valid heal action.** If a fix requires rebuilding a container (e.g., dependency change, config baked into the image, code change), the heal step in `test_<component>.py` must call `docker compose build <service>` followed by `compose_restart`. Do not shy away from rebuilds — add them as heal steps so heal.py can rebuild autonomously when needed.
