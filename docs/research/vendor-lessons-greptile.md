...
| Learning from dismissals | `bin/jjstack-review-ledger` | A per-repo dismissal ledger at `jjstack/review-memory/ledger.tsv` — **in the repo, in git, one decision per line, so a suppression is a one-line PR diff** rather than an opaque row on a vendor's server. It is the middle rung of a three-store ladder: it may demote, never suppress. |
